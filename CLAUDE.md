...
- `vastai destroy instance` は **`-y` が必須**。無いと `[y/N]` で止まり、非対話実行では
  Aborted になって課金が続く (実際にそれで生き残った)

### 失敗検知はログのパターン照合に頼らない

onstart 側に EXIT トラップを置き、成功時以外は必ず `ONSTART_FAILED` を出してから終わる。
監視側はそれを見る。以前は `Traceback|CUDA out of memory|^ERROR:` を grep していたが、
HF の `ConnectionError` がどれにも当たらず、失敗に気付けないまま **2.5時間アイドル課金**した。
想定外の形の失敗は必ず出るので、成否は番兵で判定する。

番兵すら出せずに死ぬ経路 (OOM killer / ホスト停止 / onstart が起動しない) の保険として、
ログが `STALL_MINUTES` (既定25分) 動かなければ破棄する判定も入れてある。
...
**1用語1文だと「文」を覚えて「用語」を覚えない。** v1 は用語ごとに1文 (1265文) で学習し、
train loss 0.0046 まで落ちた (=完全に暗記) のに B-WER 21.7%。取りこぼした21用語は
**全て学習データに入っていた**。文ごと暗記していただけで、用語が別の文脈に出ると読めない。
`generate_diverse_sentences.py --per-term N` で用語あたりN文を出す。

**単一話者・単一速度で合成しない。** decoder しか更新しなくても、入力の encoder 出力が
全文で同じ声から来ていると「この音響条件のときはこの語彙」の形で覚えてしまう。
日本語プリセット5声 (`jf_alpha,jf_gongitsune,jf_nezumi,jf_tebukuro,jm_kumo`) と
話速3種を録数順に巡回させる (`KOKORO_VOICES` / `KOKORO_SPEEDS`、既定で有効)。
5と3は互いに素なので周期15。CSV は同一用語の文が連続するので、これだけで
